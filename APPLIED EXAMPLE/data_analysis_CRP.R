...
dt_y <- read.csv('NC_data_0203.csv',stringsAsFactors =F)

# CRP
Y_DBP0 <- dt_y$X30710.0.0
Y_DBP2 <- dt_y$X30710.1.0

dt_nc1 <- dt[which(Y_DBP0 < 8.2 & Y_DBP0 > 0.068),] 
Y_DBP2 <- Y_DBP2[which(Y_DBP0 < 8.2 & Y_DBP0 > 0.068)]

dt_nc <- dt_nc1[(dt_nc1$enthic  %in% c(1,1001,1002,1003)),] #white people 417042
...
write.csv(basic,"basic_CRP0227.csv")

mean(Y_DBP2,na.rm=T) #2.122178
sd(Y_DBP2,na.rm=T) #3.677724

mean(dt_nc$year_of_birth,na.rm=T) #1951.365
sd(dt_nc$year_of_birth,na.rm=T) #8.045269
...
for(p in 1:length(all_x)){
  ...
  for(i in 1:ncol(dt_nc1)){
    ...
    dt_once <- cbind(X_coffee1,dt_cov,dt_nc1[,i],Y_DBP2)
...
for(i in 1:length(all_x)){
  fit1 <- lm(Y_DBP2 ~ dt_nc[,all_x[i]])
  result_DBP_asso_noadj <- rbind(result_DBP_asso_noadj,c(fit1$coef[2],summary(fit1)$coef[2,2],summary(fit1)$coef[2,4]))
...
write.csv(result_all,"result_all_CRP_0229.csv")
